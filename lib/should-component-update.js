...
function debug () {
  (console.debug || console.trace).apply(console, arguments);
}

function shouldComponentUpdate(nextProps, nextState) {
  return stateHasChanged(this.state, nextState) ||
          cursorsHaveChanged(this.props, nextProps);
}

function stateHasChanged(state, nextState) {
  if (isEqualState(state, nextState)) {
    return false;
  } else {
    if (debugEnabled) {
...
function cursorsHaveChanged(props, nextProps) {
  var cursors = filterCursors(props);
